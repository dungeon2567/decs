...
impl<U> PoolPage<U> {
    fn new() -> Self {
        Self { items: unsafe { MaybeUninit::uninit().assume_init() }, len: 0 }
    }
}
...
impl<U> Pool<U> {
    pub fn new() -> Self {
        Self { pages: unsafe { MaybeUninit::uninit().assume_init() }, len: 0 }
    }
...
impl<T: Component> Pool<Chunk<T>> {
    pub fn alloc_chunk_to_slot(&mut self, owner_index: u8, out_slot: &mut *mut Chunk<T>) -> *mut Chunk<T> {
    ...
    pub unsafe fn free_chunk(&mut self, ptr_chunk: *mut Chunk<T>) -> Option<(*mut Chunk<T>, u8)> {
        for i in 0..self.len {
            let page_ref: &mut PoolPage<Chunk<T>> = unsafe { &mut *self.pages[i].assume_init().as_ptr() };
            ...
            for idx in 0..len {
                if unsafe { page_ref.items[idx].assume_init().as_ptr() } == ptr_chunk {
                    ...
                    if idx != last {
                        ...
                        owner_idx = moved_chunk.owner_index;
                        page_ref.len -= 1;
                        debug_assert!(page_ref.len <= 32);
...
impl<T: Component> Pool<Page<T>> {
    pub fn alloc_page_to_slot(&mut self, owner_index: u8, out_slot: &mut *mut Page<T>, chunk_pool: *mut Pool<Chunk<T>>) -> *mut Page<T> {
    ...
    pub unsafe fn free_page(&mut self, ptr_page: *mut Page<T>) -> Option<(*mut Page<T>, u8)> {
        for i in 0..self.len {
            let page_ref: &mut PoolPage<Page<T>> = unsafe { &mut *self.pages[i].assume_init().as_ptr() };
            let len = page_ref.len;
            for idx in 0..len {
                if unsafe { page_ref.items[idx].assume_init().as_ptr() } == ptr_page {
                    ...
                    if idx != last {
                        ...
                        owner_idx = moved_pg.owner_index;
                        page_ref.len -= 1;
                        debug_assert!(page_ref.len <= 32);
                        return Some((moved_ptr, owner_idx));
                    } else {
