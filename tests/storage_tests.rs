...
fn test_spawn_entity_generation_and_index() {
    ...
    assert_eq!(entity1.index(), 0, "First spawned entity should have index 0");
    assert_eq!(entity1.generation(), 1, "First spawned entity should have generation 1");
    ...
    assert_eq!(entity2.index(), 1, "Second spawned entity should have index 1");
    assert_eq!(entity2.generation(), 2, "Second spawned entity should have generation 2");
    
    // Spawn third entity - should have index 2 and generation 3
    let entity3 = storage.spawn(&frame).expect("Should spawn entity");
    assert_eq!(entity3.index(), 2, "Third spawned entity should have index 2");
    assert_eq!(entity3.generation(), 3, "Third spawned entity should have generation 3");
    
    // Remove entity at index 1 (entity2)
    assert!(storage.remove(&frame, 1), "Should successfully remove entity at index 1");
    ...
    assert_eq!(entity4.index(), 1, "Spawned entity after removal should reuse index 1");
    assert_eq!(entity4.generation(), 4, "Spawned entity after removal should have generation 4 (next global generation)");
    
    // Remove entity at index 1 again
    assert!(storage.remove(&frame, 1), "Should successfully remove entity at index 1 again");
    
    // Spawn again - should reuse index 1 with next global generation
    let entity5 = storage.spawn(&frame).expect("Should spawn entity");
    assert_eq!(entity5.index(), 1, "Spawned entity after second removal should reuse index 1");
    assert_eq!(entity5.generation(), 5, "Spawned entity after second removal should have generation 5");
    
    // Remove entity at index 0
    assert!(storage.remove(&frame, 0), "Should successfully remove entity at index 0");
    ...
    assert_eq!(entity6.generation(), 6, "Spawned entity should have generation 6 (next global generation)");
    
    // Verify all entities are stored correctly
    assert_eq!(storage.get(0).copied(), Some(entity6), "Entity at index 0 should match spawned entity");
    assert_eq!(storage.get(1).copied(), Some(entity5), "Entity at index 1 should match spawned entity");
    assert_eq!(storage.get(2).copied(), Some(entity3), "Entity at index 2 should match spawned entity");
    
    // Clear changed masks before verifying invariants
    storage.clear_changed_masks();
    
    // Verify invariants
    assert!(world.verify_invariants(), "World invariants should hold after spawn operations");
}
...
fn test_spawn_multiple_entities_sequential() {
    ...
    let storage_ptr = world.get_storage::<Entity>();
    {
        ...
        for i in 0..10 {
            ...
            assert_eq!(entity.generation(), (i + 1) as u64, "Entity {} should have generation {}", i, i + 1);
            entities.push(entity);
        }

        // Verify all entities are stored
        for (i, &entity) in entities.iter().enumerate() {
            assert_eq!(storage.get(i as u32).copied(), Some(entity), "Entity at index {} should match", i);
        }

        // Remove entity at index 5 to create a tombstone
        assert!(storage.remove(&frame, 5), "Should successfully remove entity at index 5");
        ...
        assert_eq!(entity5_new.index(), 5, "Should reuse index 5 (first tombstone)");
        assert_eq!(entity5_new.generation(), 11, "Should have generation 11 (next global generation)");

        // Remove entity at index 5 again
        assert!(storage.remove(&frame, 5), "Should successfully remove entity at index 5 again");
        ...
        assert_eq!(entity5_new2.generation(), 12, "Should have generation 12 (next global generation)");
        ...
        assert_eq!(entity10.index(), 10, "Should use first empty slot after tombstones");
        assert_eq!(entity10.generation(), 13, "Should have generation 13 (next global generation)");
...
fn test_spawn_after_remove_rollback_correctness() {
    ...
    assert!(storage.rollback.verify_was_modified(0), "Should be marked as modified (Change)");
    assert!(!storage.rollback.verify_was_created(0), "Should NOT be marked as created");
    assert!(!storage.rollback.verify_was_removed(0), "Should NOT be marked as removed");
    
    // Also check that the old value (Entity 1) is stored in rollback
    let old_val = storage.rollback.get(0).expect("Should have old value stored");
    assert_eq!(old_val.generation(), 1, "Should have stored the old generation");
}
